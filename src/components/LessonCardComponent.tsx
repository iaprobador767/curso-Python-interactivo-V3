...
import { evaluatePythonCode } from '../services/geminiService'; // No longer needs isApiKeyAvailable
...
const LessonCardComponent: React.FC<LessonCardComponentProps> = ({ lesson, onMarkCompleted }) => {
  ...
  // apiKeyAvailable state is no longer needed
  ...
  const handleRunCode = async () => {
    ...
    setOutput('Ejecutando tu código (simulación)...');
    try {
      const result = await evaluatePythonCode(userCode);
      setOutput(result);
      
      // Determine output type based on simulator result
      if (result.toLowerCase().startsWith('error:')) { // Assuming simulator might prepend "Error:"
        setOutputType(OutputType.Error);
      } else if (result.trim() === "" || result.startsWith("Nota:") || result.startsWith("El código fue procesado")) {
        // Cases where the simulator ran but didn't produce "successful" user-facing output for completion
        setOutputType(OutputType.Info); 
      }
      else {
        // Any other non-empty, non-error, non-note output is considered a success
        setOutputType(OutputType.Success);
        onMarkCompleted(lesson.id); 
      }
    } catch (error) { 
      // This catch is for unexpected errors in the simulator itself, though evaluatePythonCode should handle its own logic.
      console.error("Error running code with local simulator:", error);
      setOutput(error instanceof Error ? error.message : "Ocurrió un error desconocido en el simulador.");
      setOutputType(OutputType.Error);
    }
  };
      ...
      <section className="mb-8">
        ...
        <div className="mt-4 flex flex-wrap gap-3 items-center">
          <button
            ...
            disabled={outputType === OutputType.Loading} // No longer disabled by apiKeyAvailable
